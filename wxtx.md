...
!> 使用ES时需要在网盘所在服务器上运行，或将网盘根目录映射到本地(盘符一致)。

## 密图文消息
当`esap_txd`表中有值时自动发送，`pic`，`title`，`content`字段必填。
...
## 普通图文消息
当`esap_txd`表`pic`字段无值，`title`，`digest`,`content`,`url`字段有值时自动降级为普通图文消息。
...
## 待办提醒
* 设置一个`esap ES待办提醒`计划任务，配置后缀为`.estodo`即可
...
## 数据字典
微信提醒实际是定期扫描`esap_tx`,`esap_txd`表，对应字段解析如下：
...
* 自建sql模板文件，例如建立`sql/esap/mytx.get`和`sql/esap/mytx.put`

* 参考原模板，定义新sql模板，例如`mytx.get`文件中定义`wxtx.my`，`wxtxd.my`，`mytx.put`文件中定义`wxtx.my`，输出字段与原模板一致。
