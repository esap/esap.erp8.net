...
## 数据字典
微信提醒实际是定期扫描`wxtx`表，对应字段解析如下：

|字段|描述|必填|备注|
|:----:|:--:|:--:|:----|
|cDate|创建日期|否|可设置较晚时间以延迟发送|
|flag|是否发送标识|否|1表示已发送|
|toUser|接收人|是|账号、部门或姓名，逗号隔开多个|
|toAgent|接收应用|否|默认是小助手，即为0|
|content|文字消息内容|否|可使用br或a标签|
|id|自增编号|否| |
|wi|ES工作流编号|否|用于代办通知|
|usr|ES工作流身份|否|打开代办的ES账号|
|pic|图片|否|存在时，发图片消息|
|fh|附件|否|存在时，发附件消息|
|fkid|反馈ID|否|APP反馈提醒|
|jg|消息发送结果|否|发送成功时为ok|
|rcid|ES表单系统编号|是|用于关联wxtxd表|
|safe|保密消息模式|否|1表示保密|
|db|数据库名称|否|跨账套办理工作流使用|
